        ...
        .done(function (respuestaAjax) {
            ...
            if (respuestaAjax != null) {
                if (respuestaAjax.Cuerpo != null) {
                    ...
                    if (respuestaAjax.Cuerpo.UrlImagen != null) {
                        var cuerpo = GenerarCuerpoHtml(respuestaAjax.Cuerpo);
                        $('#divData').empty();
...
function GenerarCuerpoHtml(cuerpo) {
    ...
    html.push('            <a class="image-popup-vertical-fit" data-effect="mfp-zoom-in" href="' + cuerpo.UrlImagen + '">');
    html.push('                <img class="img-responsive imagenproducto"');//style="height:180px; max-width:300px; max-height:180px;"
    html.push('                    src="' + cuerpo.UrlImagen + '" alt="' + cuerpo.UrlImagen + '">');
    html.push('            </a>');
    html.push('        </div>');
    html.push('    </div>');
    html.push('</div>');
...
function ObtenerImagenes() {
    ...
    var idCategoriaTemp = $('#ValorIdCategoria').val();
...
function DropZone() {
    ...
    var myDropzone = new Dropzone(document.body, { // Make the whole body a dropzone
        url: '../../Categoria/SubirImagenes', // Set the url
        thumbnailWidth: 80,
        thumbnailHeight: 80,
        parallelUploads: 1,
        maxFiles: 1,
    ...
    myDropzone.on("sending", function (file, xhr, formData) {
        ...
        var idCategoriaActual = $('#ValorIdCategoria').val();
        formData.append('IdCategoria', idCategoriaActual);
    ...
    myDropzone.on("queuecomplete", function (progress) {
        ...
        RecargarData();
        MensajeSuccess('Confirmación', 'Se completó la carga de imagen');
    });
