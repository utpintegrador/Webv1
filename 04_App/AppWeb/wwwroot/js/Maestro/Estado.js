...
$(document).ready(function () {
    ...
    ProcesarCargaDeEstados();
});

function ProcesarCargaDeEstados() {

    $.when(ObtenerTipoEstado())
        .done(function (respuestaTipoEstadoAjax) {

            var cboTipoEstado = $('#cboTipoEstado');
            cboTipoEstado.append($('<option/>', { value: 0, text: 'Todos' }));

            if (respuestaTipoEstadoAjax.Cuerpo != null) {
                if (respuestaTipoEstadoAjax.Cuerpo.length > 0) {
                    respuestaTipoEstadoAjax.Cuerpo.forEach(function (item, indice, array) {
...
function CargarData() {
    ...
    tb1 = $('#tb1').DataTable({
        ...
        "ajax": {
            "url": "../../Estado/ObtenerData",
            "type": "POST",
            "datatype": "json",
            "data": function (d) {
                d.Buscar = $('#txtBuscar').val();
                d.IdTipoEstado = $("#cboTipoEstado").val();
            },
...
function ObtenerConfiguracionColumnas() {
    ...
    return [
        ...
        { "data": "IdEstado", "name": "IdEstado", "visible": false },
        { "data": "Descripcion", "name": "Descripcion" },
        { "data": "DescripcionTipoEstado", "name": "DescripcionTipoEstado" },
        ...
        swalWithBootstrapButtons.fire({
            ...
            switch (respuesta.value) {
                case true:
                    ...
                    EliminarRegistro(fila.IdEstado);
                    break;
                default:
...
$(document).on('click', '#tb1 tbody #btnEditar', function () {
    var fila = tb1.row($(this).parents('tr')).data();
    if (fila !== null) {
        window.location.href = '/Estado/Edit/' + fila.IdEstado;
    }
});
