...
function CargarData() {
    ...
    tb1 = $('#tb1').DataTable({
        ...
        //},
        "ajax": {
            "url": "../../Compra/ObtenerPorIdUsuario",
            "type": "POST",
            "datatype": "json",
            "data": function (d) {
                d.IdUsuario = GetItem('IdUsuario');
                d.IdNegocioComprador = $("#cboNegocio").val();
                d.Buscar = '';
...
$(document).on('click', '#tb1 tbody #btnVisualizar', function () {
    ...
    if (fila !== null) {
        ...
        window.location.href = '/Compra/Details/' + fila.IdPedido;
                ...
                if (respuestaAjaxPorId != null) {
                    if (respuestaAjaxPorId.Cuerpo != null) {
                        ...
                        if (respuestaAjaxPorId.Cuerpo.IdEstado >= 11) {
                            ...
                            $.when(ObtenerComboComprador(respuestaAjaxPorId.Cuerpo.IdEstado))
                                .done(function (respuestaComboAjaxPorId) {
                                    if (respuestaComboAjaxPorId != null) {
                                        if (respuestaComboAjaxPorId.Cuerpo != null) {
                                            ...
                                            window._idNegocioCompradorModal = respuestaAjaxPorId.Cuerpo.IdNegocioComprador;
...
function ObtenerComboComprador(id) {
    return $.ajax({
        url: '../../Estado/ObtenerComboComprador',
        type: 'GET',
        data: {
            'idEstadoActual': id
        },
...
function ActualizarEstado() {
    ...
    var prm = {
        ...
        'IdNegocioComprador': window._idNegocioCompradorModal,
        'IdEstado': $("#cboEstadoNuevo").val()
    };

    $.ajax({
        'url': '../../Pedido/ModificarEstadoPorParteDeComprador',
        'type': 'POST',
        'data': prm,
        'dataType': 'json',
        headers: {
