...
$(document).ready(function () {
    ...
    window._idMoneda = ObtenerParametroUrl();

    $.when(ObtenerPorId(window._idMoneda))
        .done(function (respuestaPorId) {

            $('#txtDescripcion').val(respuestaPorId.Cuerpo.Descripcion);
            $('#txtSimbolo').val(respuestaPorId.Cuerpo.Simbolo);
...
function Actualizar() {
    ...
    var prm = {
        'IdMoneda': window._idMoneda,
        'Descripcion': $("#txtDescripcion").val(),
        'Simbolo': $("#txtSimbolo").val()
    };

    $.ajax({
        'url': '../../Moneda/Modificar',
        'type': 'POST',
        'data': prm,
        'dataType': 'json',
        headers: {
...
function Validar() {
    ...
    if ($("#txtSimbolo").val() == '' || $("#txtSimbolo").val() == null) {
        window._mensajeValidacion = window._mensajeValidacion + ' * El campo [Simbolo] es requerido <br/>';
    }
