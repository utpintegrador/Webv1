...
namespace Negocio.Repositorio.Maestro
{
    public class LnTipoBusqueda: Logger
    {
        ...
        {
            ResponseTipoBusquedaObtenerComboDtoApi resultado = new ResponseTipoBusquedaObtenerComboDtoApi();
            int statusCode = 0;
            try
            {
                //Dentro de AJAX => datatype: 'json', headers: {'Authorization': 'Basic ' + valor token }, ....
                ...
                if (!string.IsNullOrEmpty(response))
                {
                    resultado = JsonConvert.DeserializeObject<ResponseTipoBusquedaObtenerComboDtoApi>(response);
                }
            }
            catch (Exception ex)
            {
