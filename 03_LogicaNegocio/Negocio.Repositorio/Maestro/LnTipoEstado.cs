...
namespace Negocio.Repositorio.Maestro
{
    public class LnTipoEstado: Logger
    {
        ...
        {
            ResponseTipoEstadoObtenerComboDtoApi resultado = new ResponseTipoEstadoObtenerComboDtoApi();
            int statusCode = 0;
            try
            {
                //Dentro de AJAX => datatype: 'json', headers: {'Authorization': 'Basic ' + valor token }, ....
                ...
                if (!string.IsNullOrEmpty(response))
                {
                    resultado = JsonConvert.DeserializeObject<ResponseTipoEstadoObtenerComboDtoApi>(response);
                }
            }
            catch (Exception ex)
            {
